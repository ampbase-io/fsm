...
- `objstore_store.go` / `objstore_lease.go` / `objstore_cancel.go` / `objstore.go` — the object
  backend: Append + queries + WaitRun; leases and `lease_epoch` fencing, and the node-local
  deferral of runs that failed to resume (`deferResume`, checked in `claimEntry`); cancel
  sentinels; the S3 client, key helpers, and conditional writes.
