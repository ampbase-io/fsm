...
// started on one manager is completed by another manager sharing the bucket, the first
// manager's Wait must observe the completion instead of parking on local state that will
// never change.
func TestObjectWaitCrossManager(t *testing.T) {
	...
	start := blockingFSM(t, m1, "xwait", entered, block)

	version, err := start(ctx, "cross-1", NewRequest(&orderReq{}, &orderResp{}))
	if err != nil {
	...
	resume := completingFSM(t, m2, "xwait")
	if err := resume(ctx); err != nil {
		t.Fatalf("failed to resume on second manager: %v", err)
	}
	if err := m2.Wait(ctx, version); err != nil {
...
func TestObjectWaitByIDCrossManager(t *testing.T) {
	...
	start := blockingFSM(t, m1, "xwaitid", entered, block)

	version, err := start(ctx, "cross-2", NewRequest(&orderReq{}, &orderResp{}))
	if err != nil {
		t.Fatalf("failed to start FSM: %v", err)
	}
	<-entered
	...
	m2, _ := f.newManager(nil)
	resume := completingFSM(t, m2, "xwaitid")
	if err := resume(ctx); err != nil {
		t.Fatalf("failed to resume on second manager: %v", err)
	}
	if err := m2.Wait(ctx, version); err != nil {
